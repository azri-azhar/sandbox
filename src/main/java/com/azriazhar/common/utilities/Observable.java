...
/**
 * A simple implementation of observer pattern without boilerplate code. A listener is executed in a
 * single worker thread operating off an unbounded queue, with a timeout value to cancel the
 * execution. This is to prevent from one listener from blocking other listeners on the list.
 * Listeners are stored in a "snapshot" style thread-safe array.
 *
 * @author Mohamad Azri Azhar
 *
 * @param <T> the type of object to be passed to observers/listeners upon notification (the event
 *        source).
 */
